...
function configureScreenLogo()
{
	if(script.properties.api.screenLogo && script.teamLogo)
	{
		...
		var spriteAligner = script.properties.api.screenLogo.getSceneObject().getFirstComponent("Component.SpriteAligner");
		...
		{
			spriteAligner.bindingPoint = bindingPointPositions[script.logoPosition];

			spriteAligner.bindingPoint = new vec2( spriteAligner.bindingPoint.x + script.logoOffsetX, spriteAligner.bindingPoint.y + script.logoOffsetY );
			
			spriteAligner.size = new vec2( script.logoSize, script.logoSize );
		}
	}
}
...
function configureScreenBanner()
{
	if(script.properties.api.screenBanner && script.teamBanner)
	{
		...
		var spriteAligner = script.properties.api.screenBanner.getSceneObject().getFirstComponent("Component.SpriteAligner");
		spriteAligner.bindingPoint = new vec2( spriteAligner.bindingPoint.x, spriteAligner.bindingPoint.y + script.bannerOffsetY );
	}
}
